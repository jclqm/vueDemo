  ...
  <div class="layout">
    <Layout>
          ...
          <template v-for="(sitem, index) in menuList">
            <Submenu v-if="sitem.children && sitem.children.length" :name="sitem.name" :key="index">
              ...
              <template v-if="sitem.children && sitem.children.length">
                <MenuItem :name="`${mitem.name}`" v-for="(mitem, i) in sitem.children" :key="`${index}-${i}`"><Icon :type="mitem.meta.icon"></Icon><span>{{mitem.name}}</span></MenuItem>
              </template>
            </Submenu>
      ...
      <Layout>
        ...
        <Content :style="{margin: '20px', padding: '16px', background: '#fff', minHeight: '560px'}">
          <keep-alive>
            <router-view/>
          </keep-alive>
        </Content>
